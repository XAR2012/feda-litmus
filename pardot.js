...
/***/ function(module, exports) {
	...
	function injectIframe() {
	  var reloadInterval = arguments.length <= 0 || arguments[0] === undefined ? null : arguments[0];

	  var iframe = document.createElement('iframe');
	  iframe.id = 'litmus-html-preview-parent-iframe';
	  iframe.sandbox = 'allow-same-origin';
	  iframe.style = 'display:none;width:0;height:0;border:0;border:none;';
	  var previewParentUrl = window.location.href.replace(/(edit|send|test)/, 'preview');
	  iframe.src = previewParentUrl;
	...
	if (window.location.hostname.match('preview')) {
	  // I shouldn't be loaded in this context, but this script has been
	  // matched to all pardot subdomains. Preview behaviour should
	  // exist in pardot-preview.js
	} else {
	    if (window.location.pathname.match(/email\/.*\/edit/)) {
	      injectIframe(3000);
	    } else {
	      injectIframe();
	    }
