...
/***/ function(module, exports) {
	...
	function sanitiseHTML(htmlString) {
	  return htmlString.replace(/<script\b[^>]*>([\s\S]*?)<\/script>/gi, '');
	}
	...
	function postContent(content) {
	  ...
	  if (content) {
	    window.top.postMessage({ method: 'checklist-pardotSendContent', content: content, id: id }, '*');
	  }
	}

	if (window.location.pathname.match('previewSource')) {
	  var htmlString = sanitiseHTML(document.documentElement.outerHTML);
	  var id = emailId();
	  postContent(htmlString, id);
	}

/***/ }
/******/ ]);
//# sourceMappingURL=pardot-preview.js.map